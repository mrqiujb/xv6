...
// -1 on unsuccessful
int COW_Handler(pagetable_t pg, uint64 va, uint64 sz)
{
  pte_t *pte;
  //判断地址是否越界一个是maxva另外一个是因为cow copy的是sz之前的page 所以不能操过sz
  if (va >= MAXVA)
  {
  ...
  //获得pte
  pte = walk(pg, va, 0);
  // pte不存在或者pte无效或者不是用户pte则报错
  if (((*pte) & PTE_COW) == 0 || ((*pte) & PTE_V) == 0 || ((*pte) & PTE_U) == 0)
  {
    printf("usertrap: pte not exist or it's not cow page\n");
    return -1;
  }
  if (*pte & PTE_COW) // 是cow页面
  {
    char *mem;
    if ((mem = kalloc()) == 0) // 如果页面不够分配则结束该进程
    {
      printf("usertrap(): memery alloc fault\n");
      return -1;
    }
    memset(mem, 0, PGSIZE);
    uint64 pa = walkaddr(pg, va); //获取pa
    if (pa)
    {
      //copy内容 并设置权限
      //设置权限一般有两种一种是某位设0一种是某位设1
      //某位设1 a | 000000...000100
      //某位设0 a & 111111...111011
      //后面的位可以是PTE或者~PTE
...
void usertrap(void)
{
  ...
  { // 写页面错
    ...
    if (COW_Handler(p->pagetable, va, p->sz) == -1)
      p->killed = 1;
  }
  else if ((which_dev = devintr()) != 0)
  {
