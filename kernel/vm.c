...
void vmprint(pagetable_t page)
{
  ...
  for (int i = 0; i < 512; i++)
  {
    if (*(page + i) & PTE_V)
    {
      ...
      pagetable_t page_tem2 = (pagetable_t)PTE2PA(*(page + i));
      for (int j = 0; j < 512; j++)
      {
        if ((uint64) * (page_tem2 + j) & PTE_V)
        {
          printf(".. ..%d: pte %p pa %p\n", j, *(page_tem2 + j), PTE2PA(*(page_tem2 + j)));
          pagetable_t page_tem3 = (pagetable_t)PTE2PA(*(page_tem2 + j));
          for (int k = 0; k < 512; k++)
          {
            if ((uint64) * (page_tem3 + k) & PTE_V)
            {
              printf(".. .. ..%d: pte %p pa %p\n", k, *(page_tem3 + k), PTE2PA(*(page_tem3 + k)));
            }
          }
        }
      }
...
// 创建映射 
// 每个进程现在都有两个页表一个是内核页表copy自os的内核页表 一个是用户页表也就是从0开始的那个页表。
// 我们知道内核页表的布局 我们也知道用户也表的布局。
// 因为以前没有这套东西，如果内核需要访问用户的数据或者用户向内核写入数据都需要，先把用户的va转成pa然后从pa向内核的va写或读 参考copyin

//参数为第一个是用户页表，第二个是内核页表，第san个是终止映射地址
//其实是从0到用户申请拥有的长度，但是考虑到sbrk函数会照成地址的增加，所以从一个到另外一个以变量的形式给出

// Copy from kernel to user.
// Copy len bytes from src to virtual address dstva in a given page table.
// Return 0 on success, -1 on error.
// 从内核向用户拷贝数据
// 第一个参数为用户也表 第二个是目的地址 第三个为原地址 第四个为长度
// 根据目的地址和pagetable找到pa
// 将src的数据copy到pa上
...
// 从用户向内核拷贝 拷贝len长度的数据
// 第一个参数为用户也表 第二个是目的地址 第三个为原地址 第四个为长度
// 先根据va和pagetable找到物理地址通过walk函数 walk函数通过三级也表找到物理地址
// 将物理地址的数据移动到dst的位置
